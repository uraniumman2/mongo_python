...
function Calendar(element, instanceOptions) {
	...
	function refetchEvents() { // can be called as an API method
		destroyEvents(); // so that event are cleared before user starts waiting for AJAX
		fetchAndRenderEvents();
	}


	function renderEvents() { // destroys old event if previously rendered
		if (elementVisible()) {
			freezeContentHeight();
...
function EventManager(options) { // assumed to be a calendar
	...
	function removeEventSource(source) {
		...
		// remove all client event from that source
	...
	function removeEvents(filter) {
		...
		if (filter == null) { // null or undefined. remove all event
		...
		// Remove event from array sources.
	...
	// Modify the date(s) of an event and make this change propagate to all other event with
	// the same ID (related repeating event).
	//
	...
	function mutateEvent(event, newStart, newEnd) {
		...
		undoFunc = mutateEvents(
			clientEvents(event._id), // get event with this ID
			clearEnd,
			newAllDay,
			dateDelta,
			durationDelta
	...
	// Modifies an array of event in the following ways (operations are in order):
...
/* Tracks mouse movements over a CoordMap and raises event about which cell the mouse is over.
----------------------------------------------------------------------------------------------------------------------*/
...
DragListener.prototype = {
	...
	// Call this to stop listening to the user's mouse event
...
$.extend(Grid.prototype, {
	...
	bindHandlers: function() {
		...
		this.el.on('mousedown', function(ev) {
			if (
				...
				!$(ev.target).closest('.fc-popover').length // not on a popover (like the "more.." event one)
			) {
				_this.dayMousedown(ev);
			}
		});

		this.bindSegHandlers(); // attach event-element-related handlers. in Grid.event.js
	},
	...
	renderRangeHelper: function(start, end, sourceSeg) {
		...
		// this extra className will be useful for differentiating real event from mock event in CSS
...
$.extend(Grid.prototype, {
	...
	// Renders the given event onto the grid
	...
	// Unrenders all event. Subclasses should implement, calling this super-method first.
	...
	// When `intervalStart` and `intervalEnd` are specified, intersect the event with that interval.
		...
		var dragListener = new DragListener(view.coordMap, {
			...
			dragStop: function(ev) {
				...
				mouseFollower.stop(!hasChanged, function() {
					...
					if (hasChanged) {
						view.eventDrop(el[0], event, newStart, ev); // will rerender all event...
					}
				});
			},
			listenStop: function() {
	...
	segResizeMousedown: function(seg, ev) {
		...
		dragListener = new DragListener(this.coordMap, {
			...
			dragStop: function(ev) {
				...
				if (newEnd) {
					view.eventResize(el[0], event, newEnd, ev); // will rerender all event...
				}
			}
		});
...
function compareSegs(seg1, seg2) {
	return seg1.eventStartMS - seg2.eventStartMS || // earlier event go first
		seg2.eventDurationMS - seg1.eventDurationMS || // tie? longer event go first
		seg2.event.allDay - seg1.event.allDay || // tie? put all-day event first (booleans cast to 0/1)
...
$.extend(DayGrid.prototype, {
	...
	// Render the given event onto the Grid and return the rendered segments
	renderEvents: function(events) {
	...
	getSegs: function() {
		return (this.segs || []).concat(
			this.popoverSegs || [] // segs rendered in the "more" event popover
		);
	},
	...
	destroyEvents: function() {
		...
		this.segs = null;
		this.destroySegPopover(); // removes the "more.." event popover
	},
...
$.extend(TimeGrid.prototype, {
	...
	processOptions: function() {
		...
		this.cellDuration = snapDuration; // important to assign this for Grid.event.js
...
$.extend(TimeGrid.prototype, {
	...
	segs: null, // segment objects rendered in the component. null of event haven't been rendered yet
	eventSkeletonEl: null, // has cells with event-containers, which contain absolutely positioned event elements


	// Renders the event onto the grid and returns an array of segments that have been rendered
	renderEvents: function(events) {
...
View.prototype = {
	...
	// Handler for accepting externally dragged event being dropped in the view.
...
$.extend(BasicView.prototype, {
	...
	renderEvents: function(events) {
		...
		this.updateHeight(); // must compensate for event that overflow the row
	...
	destroyEvents: function() {
		...
		this.recordScroll(); // removing event will reduce height and mess with the scroll, so record beforehand
		this.dayGrid.destroyEvents();

		// we DON'T need to call updateHeight() because:
		// A) a renderEvents() call always happens after this, which will eventually call updateHeight()
		// B) in IE8, this causes a flash whenever event are rerendered
	},
...
$.extend(AgendaView.prototype, {
	...
	destroyEvents: function() {
		...
		// destroy the event in the subcomponents
